...
/**
 * An analyzer representing the pre/post patch binary.
 * 
 * Two used together for a summary. Fields: allClasses[str, classAttr];
 * allMethods[methodAttr]
 */
public class BinaryAnalyzer extends Analyzer {
    ...
    // final ConcurrentHashMap<SootMethod, Body> pre_bodies = new
    // ConcurrentHashMap<>();
    // final ConcurrentHashMap<SootMethod, Body> post_bodies = new
    // ConcurrentHashMap<>();
    // public Map<String, ClassAttr> preBinAttr = new HashMap<>();
    // public Map<String, ClassAttr> postBinAttr = new HashMap<>();

    /**
     * set params to fields, build CH
     * 
     * @param config
     * @param isPre
     * @throws IOException
    ...
    // * @description: extract 1) class inheritance 2) field reference 3) method
    //// call xref
    // * output format like: classname parent \t method sig \t callee list \t field
    //// xref
    // * @author: Zifan Xie
    // * @time: 2021/9/13 11:07
    // */
    // private void extractSignatures(Chain<SootClass> allClasses, final boolean
    //// isPre) {
    // int threadNum = Integer.parseInt(config.getThreadNumber());
    // final ConcurrentHashMap<SootMethod, Body> bodies = isPre ? this.pre_bodies :
    //// this.post_bodies;
    //
    // CountingThreadPoolExecutor executor = new
    //// CountingThreadPoolExecutor(threadNum,
    ...
    //// md.setStartLinenumber(method.getJavaSourceStartLineNumber()); // always
    //// return -1
